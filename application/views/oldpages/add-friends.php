 ...
 $(document).ready(function()
		  ...
		  function deleteReq(x)
  {
	...
	function addFriend(x)
	{
		...
		document.getElementById("addbtn-"+x).innerHTML='<b style="color:#fff">Loading...</b>';
		 $.post("<?php echo site_url('ajax-add-friend');?>",{"friend":x},function(output,status)
			{
				document.getElementById("addbtn-"+x).className='w3-btn w3-green';
				document.getElementById("addbtn-"+x).innerHTML='Friend Request Sent';
				
	        });
	}
	function requestAgain(x)
	{
	
		document.getElementById("reqbtn-"+x).innerHTML='<b style="color:#fff">Loading...</b>';
		 $.post("<?php echo site_url('ajax-again-friend');?>",{"friend":x},function(output,status)
			{
			
				document.getElementById("reqbtn-"+x).className='w3-btn w3-green';
				document.getElementById("reqbtn-"+x).innerHTML='Friend Request Sent';
...
<h1 class="w3-center w3-text-white" style="font-weight: 700;">Add Friends</h1>
  ...
  <table id="example1" class="table table-bordered table-striped">
				...
				foreach($users as $val)
				{
					...
					<tr>
					 ...
					 if($val['my_friend_active'] == 3 )
					 {
						 echo '<button id="addbtn-'.$val['user_id'].'" class="w3-btn w3-red" onclick="addFriend('.$val['user_id'].')">Add Friend</button>';
					 }
